...
public class BookImpl implements BookRepository {
    ...
    //private static final String GET_BOOK_TABLE_BY_ID_QUERY = "SELECT * FROM BOOKS WHERE ID =?";
    ...
    // private static final String GET_LIBRARY = "SELECT * FROM BOOKS,CATEGORIES, AUTHORS, PUBLISHER WHERE 1 = 1 AND BOOKS.ID = CATEGORIES.ID, AND BOOKS.ID = AUTHORS.ID, AND BOOKS.ID =  PUBISHERS.ID ";
    ...
    public String saveBook(Book book) {
        ...
        jdbcTemplate.update(INSERT_BOOK_QUERY, bookToSave.getId(), bookToSave.getIsbn(), bookToSave.getName(), bookToSave.getDescription());
        Book bookResponse = getByIsbnNameDescription(bookToSave.getIsbn(), bookToSave.getName(), bookToSave.getDescription());
    ...
    public Book getById(long id) {
        ...
        String getAllListQuery = ""
                ...
                + "PUBLISHERS"
                + "WHERE"
                + "BOOKS.BOOKS_ID = AUTHORS.AUTHORS_ID"
                + " OR BOOKS.BOOKS_ID = PUBLISHERS.PUBLISHERS_ID"
                + "OR BOOKS.BOOKS_ID = CATEGORIES.CATEGORIES_ID";
...
//        return jdbcTemplate.query(GET_LIBRARY, new ResultSetExtractor<List<Book>>() {
//            @Override
//            public List<Book> extractData(ResultSet rs) throws SQLException, DataAccessException {
//                List<Book> list = new ArrayList<Book>();
//                Map<Long, Book> libraryBook = new HashMap<Long, Book>();
//                Map<Long, Author> libraryAuthor = new HashMap<Long, Author>();
//                Map<Long, Category> libraryCategory = new HashMap<Long, Category>();
//                Map<Long, Publisher> libraryPublisher = new HashMap<Long, Publisher>();
//                while (rs.next()) {
//                    Long bookId = rs.getLong("Book_Key");
//                    Book book = libraryBook.get(bookId);
//                    if (book == null) {
//                        book = new Book();
//                        libraryBook.put(bookId, book);
//                        list.add(book);
//                        book.setId(bookId);
//                        book.setIsbn(rs.getString("Isbn"));
//                        book.setName(rs.getString("description"));
//
//
//                    }
//
